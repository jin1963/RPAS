 ...
 * ค่าคงที่ + ABI สำหรับ DApp ฝั่งผู้ใช้ (Public)
 * แก้เฉพาะ CONTRACT ถ้ามีการ deploy ใหม่ ที่เหลือใช้ตามนี้ได้เลย
*/
...
// ====== ADDRESSES ======
window.ADDR = {
  CONTRACT: '0x055b98549ff7622Fba30Dc324E89ecd9BA4f244E', // ✅ ที่คุณเพิ่ง deploy
  USDT:     '0x55d398326f99059fF775485246999027B3197955',  // USDT on BSC (18 decimals)
  KJC:      '0xd479ae350dc24168e8db863c5413c35fb2044ecd',  // KJC token (18 decimals)
};
...
// ====== Public Sale / Staking Contract ABI ======
window.SALE_ABI = [
...
// ====== UI FALLBACK (ใช้เมื่ออ่านแพ็กเกจ on-chain ไม่ได้) ======
const U = (n,dec=18)=> (BigInt(n) * (10n**BigInt(dec))).toString();
window.UI_CONST = {
  FALLBACK_PACKAGES: [
    // ตัวอย่างตามไอเดียช่วงราคา: 10, 100, 1000, 10000 USDT
    { id: 1, usdt: U(10, window.DECIMALS.USDT),    kjc: U(  500, window.DECIMALS.KJC) },
    { id: 2, usdt: U(100, window.DECIMALS.USDT),   kjc: U( 6000, window.DECIMALS.KJC) },
    { id: 3, usdt: U(1000, window.DECIMALS.USDT),  kjc: U(40000, window.DECIMALS.KJC) },
    { id: 4, usdt: U(10000, window.DECIMALS.USDT), kjc: U(500000, window.DECIMALS.KJC) },
  ],
};
